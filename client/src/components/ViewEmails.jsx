...
const ViewEmail = () => {
    ...
    return (
        <Box style={openDrawer ? { marginLeft: 250, width: '100%' } : { width: '100%' } }>
            <IconWrapper>
                <ArrowBack fontSize='small' color="action" cursor = "pointer" onClick={() => window.history.back() } />
                <Delete fontSize='small' color="action" cursor = "pointer" style={{ marginLeft: 40 }} onClick={()=> deleteEmail()}/>
            </IconWrapper>
            <Subject>{email.subject} <Indicator component="span">Inbox</Indicator></Subject>
            <Box style={{ display: 'flex' }}>
                <Image src={pfp} alt="profile" />
                <Container>
                    <Box>
                        <Typography>    
                            {email.to.split('@')[0]} 
